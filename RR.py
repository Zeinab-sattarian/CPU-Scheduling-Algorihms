...
def main(MODE):
    ...
    while loop:
        ...
        for i in range(len(fakelist)):
            ...
            if MODE == 4:
                    TIMER = RR(fakelist[i][0], i, flist=fakelist, plist=proglist, rlist=reportlist, timer=TIMER,
                               burst=RRQUANTOM)
...
# This runs the processes in a circular order with a fixed time quantum.
# It preempts the current process if it exceeds the time quantum or finishes its burst time.
# It takes the process ID, the index, the lists, the timer, and the time quantum as parameters,
...
def RR(process, pnumber, flist, plist, rlist, timer, burst):
    # Add Response time
    ...
    for i in range(burst):
        ...
        # If burst time == 0, exit the loop
        if flist[pnumber][3] <= 0:
            break
